...
while(cpt <= nbFiles) {
	...
	bind <- rbind(graph$NbGoHare, graph$NbGoStag)
	output <- sprintf("%scontrol_run%d.png", dossier, cpt);
	png(output)
	
	if(min == -1) {
		barplot(bind, names.arg = graph$Evaluation, beside=FALSE,names=levels(graph$Evaluation),xlab="Evaluations",ylab="Times hunted",main="Proportion of choices",col=gray.colors(2))
	} else {
		barplot(bind, names.arg = graph$Evaluation, beside=FALSE,names=levels(graph$Evaluation),xlab="Evaluations",ylab="Times hunted",main="Proportion of choices",col=gray.colors(3), ylim=c(min, max))
	}
