...
def drawHuntingTask() :
	...
	for data in dataHash :
		...
		for run in hashNbBStags.keys() :
			lastGenRun = sorted(hashNbBStags[run].keys())[-1]

			# if hashNbBStags[run][lastGenRun] > hashNbHares[run][lastGenRun] and hashNbBStags[run][lastGenRun] > 1 :
			# 	runBStags.append(run)
			if hashNbBStagsDuo[run][lastGenRun] > hashNbSStags[run][lastGenRun] and hashNbBStagsDuo[run][lastGenRun] > 1 :
				runBStags.append(run)

		hashRunBStags.append(runBStags)
	...
	legend = plt.legend(['Control', 'Single NN', 'Two NN with network change'], loc = 1, frameon=True)
	...
	# # legend = plt.legend(['Control Single NN', 'Control Two NN', 'Single NN', 'Two NN'], loc = 1, frameon=True)
	# # frame = legend.get_frame()
	# # frame.set_facecolor('0.9')
	# # frame.set_edgecolor('0.9')
	...
	for data in dataHash :
		...
		for run in hashFitness.keys() :
			if run in [16, 17] :
				...
				axe1.set_ylim(0, 5000)
				axe1.set_ylabel('Fitness')

				plt.savefig(outputData + "/fitnessRun" + str(run) + ".png", bbox_inches = 'tight')
				plt.savefig(outputData + "/fitnessRun" + str(run) + ".svg", bbox_inches = 'tight')
				plt.close()
