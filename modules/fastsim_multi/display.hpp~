...
namespace fastsim
{
  ...
  public:
    ...
    void add_dead_prey(float x, float y, float radius, int color) { struct Position pos; pos.x = x; pos.y = y; pos.radius = radius; pos.color = color; _dead_preys_list.push_back(pos); }
    void display_dead_prey(float x, float y, float radius, int color);
#else
  ...
  protected:
    ...
    std::vector<struct Position> _end_positions;
#endif
  };
}
#endif	    /* !SDL_HH_ */
