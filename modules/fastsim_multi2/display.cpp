...
namespace fastsim
{
  ...
  void Display :: _blit_map()
  {
      ...
      for (unsigned j = 0; j < _map->get_pixel_h(); ++j)
      {
		...
		for(size_t i = 0; i < _map->get_walls().size(); ++i)
		{
			const Wall& w = _map->get_walls()[i];

			int x1_w = _map->real_to_pixel(w.get_x1());
			int y1_w = _map->real_to_pixel(w.get_y1());
			int x2_w = _map->real_to_pixel(w.get_x2());
			int y2_w = _map->real_to_pixel(w.get_y2());

			for(unsigned j = y1_w; j <= y2_w; ++j)
				for(unsigned k = x1_w; k <= x2_w; ++k)
				{
					_put_pixel(_map_bmp, k, j, 0, 0, 0);
				
#ifdef NO_VIDEO
					_put_pixel(_map_bmp2, k, j, 0, 0, 0);
#endif					
				}
		}
...
/*  // Doesn't seem to bother it if x0, y0, x1 or x1 are out
  ...
  void Display :: _disp_switches()
  {
    for (size_t i = 0; i < _map->get_illuminated_switches().size(); ++i)
    {
			...
			Uint32 color = _color_from_id(_screen, sw.get_display_color());
				...
				for (size_t i = 0; i < _robots[j]->get_lasers().size(); ++i)
				{
					...
					unsigned y_laser = _map->real_to_pixel(_robots[j]->get_pos().y() 
												 ...
												 * sinf(_robots[j]->get_pos().theta()
													+ _robots[j]->get_lasers()[i].get_gap_angle()));
					// std::cout << x_laser << "," << y_laser << "/" << _robots[j]->get_lasers()[i].get_x_pixel() << "," << _robots[j]->get_lasers()[i].get_y_pixel() << std::endl;
...
//			unsigned int col=_robots[i]->color();
		  ...
		  if(i == _selected || (i == (_selected + 1) % _robots.size()))
		  {
		  	...
		  	status = _robots[i]->get_display_status("circle_int");
		  	if(status >= 0)
		  		_circle(_screen, x, y, r - 2, _color_from_id(_screen, status));

		  	status = _robots[i]->get_display_status("cross");
		  	if(status >= 0)
		  	{
