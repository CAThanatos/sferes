...
namespace fastsim
{
  ...
  public:
    ...
    status_t get_pixel(unsigned x, unsigned y) const 
    {
      return (y * _h + x >= 0 && y * _h + x < _data.size()) ? 
				_data[y * _h + x]	: _data[_data.size()-1]; 
    }
    ...
    //
    bool check_inter_ray_circle(float x1, float y1,
         float x2, float y2,
         float xr, float yr, float radius) const;

    bool check_collision(float x, float y, float radius) const;
		       
		typedef struct Return_check_inter
		{
			int pixel;
			float dist;
		} Return_check_t;
		       
    Return_check_t check_inter_is_dist(float x1, float y1,
		       float x2, float y2);
    Return_check_t check_inter_angle(float x1, float y1,
          float angle, float range);
    
    const std::vector<Goal>& get_goals() const { return _goals; }
    void add_goal(const Goal& g) { _goals.push_back(g); }

    const std::vector<Wall>& get_walls() const { return _walls; }
    void add_wall(const Wall& w) { _walls.push_back(w); }
    ...
    void remove_illuminated_switch(ill_sw_t is)
    {
    	for(int i = 0; i < _illuminated_switches.size(); ++i)
    		if(&(*(_illuminated_switches[i])) == &(*is))
    		{
    			_illuminated_switches.erase(_illuminated_switches.begin() + i);
    		}
    }
...
#ifdef LOG_BEHAVIOR
		void log_robot_position(float x, float y)
		{
			int _x = real_to_pixel(x);
    	int _y = real_to_pixel(y);
    	
      if (_y * _h + _x >= 0 && _y * _h + _x < _behaviour_map.size()) 
      {
      	if(_behaviour_map[_y * _h + _x].color != 1) 
      	{
      		_behaviour_map[_y * _h + _x].color = 1;
      		_behaviour_map[_y * _h + _x].force = 1;
      	}
      	else
      	{
      		_behaviour_map[_y * _h + _x].force++;
      	}
      }
 		}
#endif
  ...
  protected:
    ...
    std::vector<status_t> _data_save;
    ...
    Posture _pos_landmark;
		...
		std::vector<struct Behaviour_map_cell> _behaviour_map;
#endif
  };
}
