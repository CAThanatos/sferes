...
#ifdef TEST_PROBA_COOP
			std::string fileProbaName = "./" + res_dir + "probaCoop" + boost::lexical_cast<std::string>(Params::simu::total_preys) + ".txt";
			std::ofstream fileProbaCoop(fileProbaName.c_str(), std::ios::out | std::ios::app);
			fileProbaCoop << "NbHunts,NbCooperativeHunts,NbIterations" << std::endl;
			float nb_coop_hunt = 0;
			float nb_total_hunt = 0;
...
/*				simu_t simu(map);
...
#ifdef TEST_PROBA_COOP
			nb_total_hunt += moy_hares1;
			nb_coop_hunt += (moy_hares1 - moy_hares1_solo);
#endif
      ...
      moy_hares2_solo /= nb_encounters;
     	...
     	food1 /= nb_encounters;
			...
			fileProbaCoop << nb_total_hunt << "," << nb_coop_hunt << ",1000" << std::endl;
			fileProbaCoop.close();
#endif
