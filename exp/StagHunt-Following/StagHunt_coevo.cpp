...
#define COEVO
#line 1 "/home/asriellursat/sferes2-0.99/exp/StagHunt-Following/StagHunt.cpp"
...
#if defined(BEHAVIOUR_VIDEO) && !defined(TRACE_ONLY)
	   	...
	   	void check_status(Simu &simu)
   	{
   		...
   		for(int i = 0; i < _vec_waypoints.size(); ++i)
   		{
   			...
   			for(int j = 0; j < simu.robots().size(); ++j)
   			{
	   			...
	   			if(!hunter->get_waypoints()[_vec_waypoints[i]->get_id()])
	   			{
		   			...
		   			if(dist < _vec_waypoints[i]->get_radius())
		   			{
		   				...
		   				hunter->add_waypoint(_vec_waypoints[i]->get_id());
		   			}
	   			}
   			}
   		}
