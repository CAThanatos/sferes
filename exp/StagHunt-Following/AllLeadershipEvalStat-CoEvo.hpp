				...
				void refresh(const E& ea)
      {
        ...
        this->_create_log_file_leadership(ea, "allleadershipevalstat.dat");
          ...
          for(int i = 0; i < ea.pop().size(); ++i)
          {
            ...
            float proportion_leader_first = ea.pop()[i]->nb_leader_first()/ea.pop()[i]->nb_waypoints_coop();
            (*this->_log_file_leadership) << "," << proportion_leader_first;
            (*this->_log_file_leadership) << "," << ea.pop()[i]->nb_leader_first() << "," << ea.pop()[i]->nb_waypoints_coop() << std::endl;
          }
            ...
            float proportion_leader_first = ea.pop_co()[i]->nb_leader_first()/ea.pop_co()[i]->nb_waypoints_coop();
            (*this->_log_file_leadership_co) << "," << proportion_leader_first;
            (*this->_log_file_leadership_co) << "," << ea.pop_co()[i]->nb_leader_first() << "," << ea.pop_co()[i]->nb_waypoints_coop() << std::endl;
					}
				}
      }
