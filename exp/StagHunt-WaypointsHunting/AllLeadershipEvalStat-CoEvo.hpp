				...
				void refresh(const E& ea)
      {
        ...
        this->_create_log_file_leadership(ea, "allleadershipevalstat.dat");
          ...
          for(int i = 0; i < ea.pop().size(); ++i)
          {
            ...
            (*this->_log_file_leadership_co) << "," << ea.pop_co()[i]->nb_leader_preys_first() << "," << ea.pop_co()[i]->nb_preys_killed_coop();

            float proportion_leader_waypoints_first = ea.pop_co()[i]->nb_leader_waypoints_first()/ea.pop_co()[i]->nb_preys_killed_coop();
            (*this->_log_file_leadership_co) << "," << proportion_leader_waypoints_first;
            (*this->_log_file_leadership_co) << "," << ea.pop_co()[i]->nb_leader_waypoints_first() << "," << ea.pop_co()[i]->nb_preys_killed_coop() << std::endl;
					}
				}
      }
